# ============================================================
# Energy Usage Prediction - Streamlit App (Professional UI)
# ============================================================
...
<div class="overview-box">
    <p class="subtitle">
        ...
        The system analyzes factors such as occupancy, outdoor temperature, solar irradiance, and day patterns 
        to estimate hourly or daily energy usage. It also calculates the associated <b>carbon footprint</b>, 
        helping facilities optimize power utilization and support sustainability goals.
    </p>
</div>
""", unsafe_allow_html=True)
...
with col1:
    if st.button("Predict for Whole Day", use_container_width=True):
        st.session_state["selected_mode"] = "whole_day"
with col2:
    if st.button("Predict for Specific Hour", use_container_width=True):
...
if "predict" in st.session_state and st.session_state["predict"]:
    ...
    if st.session_state["selected_mode"] == "specific_hour":
        ...
        df_results = pd.DataFrame({
            ...
            "Predicted Usage (kWh)": y_pred_flat.round(2)
        })
